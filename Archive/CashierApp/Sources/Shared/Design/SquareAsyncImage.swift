...
struct SquareAsyncImage: View {
    ...
    // Slight overscan to hide white margins inside source images (1.0 = none)
    var overscan: CGFloat = 1.0

    // Cache the last successful image to avoid flicker/disappear when the view re-renders
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack {
                // Base placeholder background to reserve space and keep layout stable
                ...
                if let url = url {
                    AsyncImage(url: url, transaction: txn) { phase in
                        switch phase {
                        case .empty:
                            // Show cached image if the URL matches; prevents disappearance during reloads
                        ...
                        case .success(let img):
                            img
                                ...
                                .transition(fade)
                                .onAppear {
                                    // Cache the successful image for this URL
