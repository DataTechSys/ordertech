...
// Provide a manifest for clients: if local token, synthesize; else proxy upstream
app.get('/manifest', async (req, res) => {
  try {
    const tok = req.header('x-device-token');
    if (tok && LOCAL.devices.has(tok)) {
      const d = LOCAL.devices.get(tok) || {};
      // Return a minimal manifest compatible with client expectations
      return res.json({
        brand: { display_name: 'Company' },
        profile: {
          display_name: d.name || 'Display',
          name: d.name || 'Display',
          branch: d.branch || null,
          tenant_name: 'Company',
          short_code: (d.tenant_id || '').slice(0, 6)
        }
      });
    }
    return proxyJSON(req, res, '/manifest', { ttl: 60 });
  } catch (e) {
    return res.status(500).json({ error: 'server_error' });
  }
});
...
// Association helper: return tenant_id for locally-issued tokens; otherwise proxy upstream
app.post('/ws/associate', async (req, res) => {
  try {
    const bearer = (req.header('authorization') || '').trim();
    const tokenHeader = (req.header('x-device-token') || '').trim();
    const tok = tokenHeader || (bearer.toLowerCase().startsWith('bearer ') ? bearer.slice(7) : '');
    if (tok && LOCAL.devices.has(tok)) {
      const d = LOCAL.devices.get(tok) || {};
      return res.json({ tenant_id: d.tenant_id || '' });
    }
    const body = JSON.stringify(req.body || {});
    const { status, headers, body: buf } = await adminRequest('POST', '/ws/associate', req, { body });
...
app.post('/device/pair/register', async (req, res) => {
  try {
    ...
    // Forced local claim (always claim on proxy, useful for testing)
    if ((process.env.PROXY_FORCE_LOCAL_CLAIM || '').toLowerCase() === 'true') {
      const token = issueLocalDevice(tenant_id, role, name, branch, code);
      return res.json({ status: 'claimed', device_token: token, tenant_id, role, code });
    }
    ...
    // If upstream completely fails and we are forcing local claim, still issue a token
    if ((process.env.PROXY_FORCE_LOCAL_CLAIM || '').toLowerCase() === 'true') {
      const input = req.body || {};
      const tenant_id = input.tenant_id || req.header('x-tenant-id') || '';
      const role = input.role || 'display';
