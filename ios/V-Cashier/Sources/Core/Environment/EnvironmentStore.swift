...
final class EnvironmentStore: ObservableObject {
    ...
    // Foodics API bearer token (Keychain)
    @Published var foodicsToken: String? = nil {
        didSet { KeychainStore.standard.set(foodicsToken, key: Keys.foodicsToken) }
    }
    @Published var requireActivation: Bool = true {
        didSet { persist() }
    }
    // Offline menu preview (read-only); when true, app shows cached menu even without activation
    @Published var previewMenu: Bool = false {
        didSet { UserDefaults.standard.set(previewMenu, forKey: Keys.previewMenu) }
    }
    ...
    private struct Keys {
        ...
        static let foodicsToken = "Env.foodicsToken"
        static let previewMenu = "Env.previewMenu"
    }
    ...
    init() {
        ...
        self.foodicsToken = KeychainStore.standard.string(forKey: Keys.foodicsToken)
        // Preview toggle persisted
        self.previewMenu = UserDefaults.standard.bool(forKey: Keys.previewMenu)
