...
struct DisplayPickerView: View {
    ...
    // Track which display we are connecting to in order to show a spinner and disable others
    @State private var connectingId: String? = nil
...
ForEach(visible) { d in
                                    ...
                                    let isConn = isCurrentConnected(d)
                                    let rowBG = isConn ? Color.red.opacity(0.85) : (state.isAvailable ? Color.green.opacity(0.85) : state.bg)
                                    let tagText = isConn ? "Hang Up" : (state.isAvailable ? "Call" : state.label)
                                    Button(action: { Task { if isConn { await hangup() } else { await start(with: d.id) } } }) {
VStack(alignment: .leading, spacing: 6) {
                                            HStack(spacing: 12) {
                                                ...
                                                Text(tagText)
                                                    ...
                                                    .padding(.vertical, 4)
                                                    .background(Capsule().fill(isConn ? Color.white.opacity(0.95) : Color.white.opacity(0.85)))
                                                    .foregroundColor(isConn ? .black : .black)
                                            }
                                    ...
                                    .disabled((!state.isAvailable && !isConn) || (connectingId != nil && connectingId != d.id))
                                }
                            }
                        }
                    }
