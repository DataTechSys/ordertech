...
struct ModifiersView: View {
    ...
    var body: some View {
        ...
        .toolbar {
            ToolbarItem(placement: .cancellationAction) {
                Button(action: { dismiss(); onCancel() }) { Image(systemName: "xmark") }
    ...
    private var modifierGroupsSections: some View {
        ForEach(groups) { g in
            Section(header: Text(g.group.name)) {
                ModifierGroupGrid(group: g, selection: $selection, toggleAction: toggleOption)
    ...
    private struct ModifierGroupGrid: View {
        let group: AnyCodableModifierGroup
        @Binding var selection: [String: Set<String>]
        let toggleAction: (AnyCodableModifierGroup.Option, AnyCodableModifierGroup) -> Void
        ...
        var body: some View {
            ...
            LazyVGrid(columns: columns, alignment: .leading, spacing: 8) {
                ForEach(opts) { opt in
                    ...
                    Button(action: { toggleAction(opt, group) }) {
                        HStack(spacing: 6) {
                            Text(opt.name)
