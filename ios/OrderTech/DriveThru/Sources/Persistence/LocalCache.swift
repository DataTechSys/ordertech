...
enum LocalCache {
    ...
    static func clearCache() {
        // Delete cached files
        try? delete("products.json")
        try? delete("categories.json")
        try? delete("modifier_reference_table.json")
        // Clear last sync date to force refresh
        lastSyncDate = nil
        print("[LocalCache] Cache cleared")
    }
}
