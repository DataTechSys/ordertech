...
if let tid = await self.associateViaWSHost(env: env) { env.setTenantId(tid) }
            ...
            if displayName == nil || displayName!.isEmpty {
                if let prof = try? await http.fetchDeviceProfile() {
                    if let dn = prof.display_name?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines), !dn.isEmpty { displayName = dn }
                    if (branchName == nil || branchName!.isEmpty), let br = prof.branch?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines), !br.isEmpty { branchName = br }
                    if shortId == nil, let sc = prof.short_code?.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines) {
            ...
            // Sync device_id from server if provided in manifest profile
            if let dict = parsed as? [String: Any],
               let profile = dict["profile"] as? [String: Any],
               let deviceIdFromServer = profile["device_id"] as? String,
               !deviceIdFromServer.isEmpty {
                print("[Activation] Syncing device_id from server: \(deviceIdFromServer)")
                let oldDeviceId = app.deviceId
                app.deviceId = deviceIdFromServer
                // Update keychain to persist the server's device_id
                DeviceIdStore.shared.updateDeviceId(deviceIdFromServer)
                
                // If device_id changed, we need to notify that app model changed
                if oldDeviceId != deviceIdFromServer {
                    print("[Activation] Device ID changed from \(oldDeviceId) to \(deviceIdFromServer), triggering objectWillChange")
                    app.objectWillChange.send()
                }
            }
            
            // Sync Foodics token from server if provided in manifest profile
            if let dict = parsed as? [String: Any],
               let profile = dict["profile"] as? [String: Any] {
                // Try multiple field variants for Foodics token
                let tokenCandidates: [String?] = [
                    profile["foodics_token"] as? String,
                    profile["foodicsToken"] as? String,
                    profile["foodics_api_token"] as? String,
                    dict["foodics_token"] as? String
                ]
                if let foodicsToken = tokenCandidates.compactMap({ $0 }).first(where: { !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty }) {
                    let trimmed = foodicsToken.trimmingCharacters(in: .whitespacesAndNewlines)
                    let oldToken = env.foodicsToken ?? ""
                    if trimmed != oldToken {
                        print("[Activation] Syncing Foodics token from server (\(trimmed.prefix(20))...)")
                        env.foodicsToken = trimmed
                    }
                }
            }
