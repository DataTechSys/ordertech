...
struct RootView: View {
    ...
    #endif
    @AppStorage("OT.display.externalRotation") private var externalRotationRaw: String = "none"
...
// Removed top-right hot corner to avoid gesture conflicts over the bill's first row
            ...
            .onChange(of: app.deviceId) { newDeviceId in
                print("[UI] deviceId changed → \(newDeviceId)")
                Task { @MainActor in sessionStoreHolder.ensure(env: env, app: app) }
            }
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            StatusChipView(status: statusChipText, provider: nil, compact: true, dotOnly: true)
    ...
    private var content: some View {
        Group {
            ...
            if let s = sessionStoreHolder.store, env.deviceToken != nil {
                ...
                // Show content on main iPhone screen
                displayHomeView
                    .onAppear {
                        // Also present content on external display if connected
                        if externalDisplayManager.isExternalDisplayConnected {
                            var contentWithEnvironment = AnyView(
                                displayHomeView
                            ...
                            // Add WebRTC environment objects if available
                            ...
                            externalDisplayManager.presentOnExternalDisplay(
                                content: contentWithEnvironment,
                                orientationModel: orientation
                            )
                        }
                    }
                    .onDisappear {
                        // Disconnect external display when main content disappears
                        externalDisplayManager.disconnectExternalDisplay()
                    }
                    .onChange(of: externalDisplayManager.isExternalDisplayConnected) { isConnected in
                        if isConnected {
                            // External display connected - present content with all environment objects
                            var contentWithEnvironment = AnyView(
                                displayHomeView
                                    .environmentObject(env)
                                    .environmentObject(app)
                            ...
                            // External display disconnected - clean up
                            externalDisplayManager.disconnectExternalDisplay()
                        }
                    }
.onChange(of: externalRotationRaw) { _ in
                        // Re-present external display with new rotation setting
                        if externalDisplayManager.isExternalDisplayConnected {
                            var contentWithEnvironment = AnyView(
                                displayHomeView
                                    .environmentObject(env)
