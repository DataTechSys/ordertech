...
ADD COLUMN IF NOT EXISTS cashier_device_id UUID NULL;
...
CREATE TABLE IF NOT EXISTS device_connection_events (
  ...
  device_id UUID NOT NULL, -- Fixed: UUID instead of TEXT
  ...
  peer_device_id UUID NULL, -- Fixed: UUID instead of TEXT
...
CREATE OR REPLACE FUNCTION update_device_connection_status(
  p_tenant_id UUID,
  p_device_id UUID, -- Fixed: UUID instead of TEXT
  p_connection_status TEXT,
  p_session_id TEXT DEFAULT NULL,
  p_peer_device_id UUID DEFAULT NULL, -- Fixed: UUID instead of TEXT
...
BEGIN
  ...
  SET 
    ...
    END,
    cashier_name = COALESCE(p_cashier_name, cashier_name), -- Keep existing cashier_name if not provided
    cashier_device_id = CASE 
      WHEN p_peer_role = 'cashier' AND p_peer_device_id IS NOT NULL THEN p_peer_device_id
      WHEN p_connection_status = 'offline' THEN NULL
...
RETURNS TABLE (
  device_id UUID, -- Fixed: UUID instead of TEXT
  name TEXT,
  role TEXT,
  branch TEXT,
  branch_id UUID,
