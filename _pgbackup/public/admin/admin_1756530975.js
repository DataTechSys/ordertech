(() => {
  ...
  (function () {
    ...
    function wireCollapsibles(root){
      ...
      root.querySelectorAll('[data-section]').forEach(sec => {
        ...
        if (!isOpen && body) { body.hidden = true; if (chev) chev.textContent = '▸'; }
        head?.addEventListener('click', (ev) => {
          ev.stopPropagation();
          const expanded = head.getAttribute('aria-expanded') === 'true';
          if (!expanded) {
            // Close other open sections (accordion)
            root.querySelectorAll('[data-section].open').forEach(other => {
              if (other === sec) return;
              const oHead = other.querySelector('.menu-head');
              const oBody = other.querySelector('.menu-body');
              const oChev = oHead?.querySelector('.chev');
              if (oHead) oHead.setAttribute('aria-expanded', 'false');
              if (oBody) oBody.hidden = true;
              if (oChev) oChev.textContent = '▸';
      ...
      // Optional accordion behavior for submenus (if present in markup)
      root.querySelectorAll('[data-submenu]').forEach(sm => {
        ...
        head?.addEventListener('click', (ev) => {
          ...
          if (!expanded) {
            // Close siblings
            const container = sm.parentElement || root;
            container.querySelectorAll('[data-submenu]').forEach(other => {
              if (other === sm) return;
              const oHead = other.querySelector('.submenu-head');
              const oBody = other.querySelector('.submenu-body');
              const oChev = oHead?.querySelector('.chev');
              if (oHead) oHead.setAttribute('aria-expanded', 'false');
              if (oBody) oBody.style.display = 'none';
              if (oChev) oChev.textContent = '▸';
            });
          }
          head.setAttribute('aria-expanded', String(!expanded));
          if (body) body.style.display = expanded ? 'none' : 'grid';
          if (chev) chev.textContent = expanded ? '▸' : '▾';
        });
      });
    }
  ...
  const tenantNameCrumb = document.getElementById('tenantNameCrumb');
  ...
  async function fetchTenants(){
    ...
    renderTenantList(arr); try { const opt = tenantSel?.selectedOptions?.[0]; if (tenantNameCrumb) tenantNameCrumb.textContent = opt ? (opt.textContent.split(' (')[0] || '—') : '—'; } catch {}
  ...
  tenantSel?.addEventListener('change', async () => { await refreshDomains(); await refreshBranding(); await refreshLicenseAndDevices(); await refreshBranches(); await refreshDisplayState(); await refreshDashboard(); await refreshBilling(); await loadCategoriesList(); await loadProductsList(); try { const opt = tenantSel?.selectedOptions?.[0]; if (tenantNameCrumb) tenantNameCrumb.textContent = opt ? (opt.textContent.split(' (')[0] || '—') : '—'; } catch {} });
  ...
  async function refreshBranding(){
    ...
    try {
      // Update sidebar logo image dynamically from brand settings, with a safe fallback
      const logoEl = document.querySelector('.sidebar .logo');
      if (logoEl) {
        const brandSrc = (logoUrl.value || '').trim();
        if (brandSrc) {
          logoEl.style.backgroundImage = `url('${brandSrc}')`;
        } else {
          logoEl.style.backgroundImage = "url('/public/ordertech.jpg'), url('/public/ordertech.ico')";
        }
    ...
    try { if (tenantNameCrumb && (!tenantSel || !tenantSel.value)) tenantNameCrumb.textContent = (brandName.value || '—'); } catch {}
  }
  ...
  async function loadCategoriesList(){
    try {
      ...
      if (categoryTableWrap){
        ...
        // category actions
        categoryTableWrap.querySelectorAll('[data-act="rename-cat"]')?.forEach(btn => {
          btn.addEventListener('click', async (ev) => {
            ev.stopPropagation();
            const id = btn.getAttribute('data-id');
            const cat = CATEGORIES_CACHE.find(c => c.id === id);
            const nn = prompt('Rename category', cat?.name || ''); if (!nn) return;
            const t = tenantSel?.value || CURRENT_TENANT_ID; if (!t) { alert('Missing tenant'); return; }
            const r = await fetch(`/admin/tenants/${t}/categories/${encodeURIComponent(id)}`, { method:'PUT', headers: adminHeaders(), body: JSON.stringify({ name: nn }) });
            if (!r.ok) { const e = await r.json().catch(()=>({})); alert('Rename failed: ' + (e.error || r.status)); return; }
            await loadCategoriesList();
          });
        });
        categoryTableWrap.querySelectorAll('[data-act="delete-cat"]')?.forEach(btn => {
          btn.addEventListener('click', async (ev) => {
            ev.stopPropagation();
            const id = btn.getAttribute('data-id');
            if (!confirm('Delete this category? Categories with products cannot be deleted.')) return;
            const t = tenantSel?.value || CURRENT_TENANT_ID; if (!t) { alert('Missing tenant'); return; }
            const r = await fetch(`/admin/tenants/${t}/categories/${encodeURIComponent(id)}`, { method:'DELETE', headers: adminHeaders() });
            if (!r.ok) { const e = await r.json().catch(()=>({})); alert('Delete failed: ' + (e.error || r.status)); return; }
            await loadCategoriesList();
          });
        });
