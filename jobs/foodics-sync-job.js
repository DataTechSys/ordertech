#!/usr/bin/env node
// Cloud Run Job: Sync Foodics orders
...
async function syncOrders() {
  ...
  try {
    console.log(`🔄 Foodics Sync Job started - ${new Date().toISOString()}`);
    ...
    const MAX_PAGES = 20; // Safety limit: fetch max 20 pages (1000 orders)
    
    // Foodics API filters are broken, so we fetch by sort order
    ...
    for (const order of allOrders) {
      try {
        ...
        // Fetch order items using dedicated endpoint
        ...
        if (itemsResponse.status === 200) {
          ...
          console.log(`   ⚠️  No items for order ${order.reference}`);
        }
